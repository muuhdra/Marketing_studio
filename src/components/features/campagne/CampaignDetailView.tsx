...
import { actionGetDnaSignedUrl } from '@/lib/actions/dna'
import { useToast } from '@/lib/stores/toastStore'
import { useMediaStore, engineLabel, typeLabel } from '@/lib/stores/mediaStore'
...
const CONTENT_LABELS: Record<string, { label: string; color: string }> = {
  ugc:        { label: 'UGC Vidéo',      color: 'text-accent border-accent bg-accent/10'        },
  commercial: { label: 'Commercial',     color: 'text-purple border-border-purple bg-purple/10'  },
  shooting:   { label: 'Shooting Photo', color: 'text-teal border-border-teal bg-teal/10'        },
  visuel:     { label: 'Visuels',        color: 'text-amber border-amber/40 bg-amber/10'         },
}
...
export default function CampaignDetailView({ data }: Props) {
  const { campaign: initial, dna, contentTypes, assignments } = data
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            <Link href="/campagnes" className="font-sans text-[11px] text-text-dim hover:text-text-secondary transition-colors">
              ← Campagnes
            </Link>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="danger" size="sm" onClick={() => setConfirmDelete(true)}>Supprimer</Button>
        </div>
      </div>

      {/* ── Timeline progress ── */}
      {campaign.start_date && campaign.end_date && (
        <div className="bg-bg-card border border-border rounded-neo-lg px-5 py-4 mb-5">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              <span className="font-sans text-[10px] text-text-dim">{formatDateShort(campaign.start_date)}</span>
              ...
              <span className="font-sans text-[10px] text-text-dim">{formatDateShort(campaign.end_date)}</span>
            </div>
            <div className="flex items-center gap-3 ml-4">
              <span className="font-sans text-[11px] font-bold text-text-primary">{progress}%</span>
              {left !== null && campaign.status === 'active' && (
                <span className={`font-sans text-[10px] font-bold px-2 py-0.5 rounded-neo border
                  ${left <= 7 ? 'text-coral border-coral/40 bg-coral/5' : 'text-teal border-border-teal bg-teal/5'}`}>
                  J-{left}
                </span>
              )}
              {left !== null && left < 0 && (
                <span className="font-sans text-[10px] text-text-dim border border-border rounded-neo px-2 py-0.5">
                  Terminée
                </span>
              )}
            </div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {[
          ...
          <div key={p.label} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-neo border font-sans text-[11px] font-bold transition-all
            ${p.active ? p.color : 'text-text-dim border-fg/40 opacity-40'}`}>
      ...
      {campaign.status === 'draft' && !wizardComplete && (
        <div className="mb-5 bg-amber/5 border border-amber/40 rounded-neo-lg px-5 py-4 flex items-center justify-between">
          <div>
            <p className="font-sans text-[12px] font-bold text-amber mb-0.5">⚠ Configuration incomplète</p>
            <p className="font-sans text-[11px] text-text-dim">
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              { label: 'Pré-camp.',    value: campaign.pre_campaign_enabled
                  ? (campaign.pre_campaign_start
                      ? `${formatDateShort(campaign.pre_campaign_start)} → ${campaign.pre_campaign_end ? formatDateShort(campaign.pre_campaign_end) : '…'}`
                      : 'Activée')
                  : 'Désactivée' },
              { label: 'Post-camp.',   value: campaign.post_campaign_enabled
                  ? (campaign.post_campaign_delay_weeks
                      ? `+${campaign.post_campaign_delay_weeks} sem. après la fin`
                      : 'Activée')
                  : 'Désactivée' },
            ].map((r) => (
              <div key={r.label} className="flex justify-between items-baseline border-b border-fg/40 pb-2 last:border-0 last:pb-0">
                <span className="font-sans text-[10px] text-text-dim">{r.label}</span>
                <span className="font-sans text-[11px] font-bold text-text-secondary">{r.value}</span>
              </div>
            ))}
            {campaign.pre_campaign_enabled && campaign.pre_campaign_dna && (
              <div className="mt-1 bg-purple/5 border border-border-purple rounded-neo p-2.5">
                <p className="font-sans text-[9px] font-bold text-purple mb-1">DIRECTION PRÉ-CAMPAGNE</p>
                <p className="font-sans text-[10px] text-text-secondary leading-relaxed line-clamp-3">
                  {campaign.pre_campaign_dna}
                </p>
              </div>
            )}
          </div>
        ...
        {/* ADN de la campagne (le CŒUR) */}
        <div className="bg-bg-card border border-border rounded-neo-lg p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <p className="nb-label">ADN de la Campagne</p>
              <span className="font-sans text-[9px] font-bold text-teal border border-border-teal px-2 py-0.5 rounded-neo">CŒUR</span>
            </div>
            <div className="flex items-center gap-2">
              {typeof dna?.health_score === 'number' && (
                <span className={`font-sans text-[10px] font-bold px-2 py-0.5 rounded-neo border
                  ${dna.health_score >= 70 ? 'text-teal border-border-teal bg-teal/10'
                    : dna.health_score >= 40 ? 'text-amber border-amber/40 bg-amber/10'
                    : 'text-coral border-coral/40 bg-coral/10'}`}>
                  Santé {dna.health_score}/100
                </span>
              )}
              {dna?.raw_content && (
                <span className="font-sans text-[10px] font-bold text-teal">v{dna.version}</span>
              )}
            </div>
          </div>
          {dna?.raw_content ? (
            <>
              <div className="max-h-[200px] overflow-y-auto bg-bg-surface border border-border rounded-neo p-3">
                <p className="font-sans text-[11px] text-text-secondary whitespace-pre-wrap leading-relaxed">
                  {dna.raw_content}
                </p>
              </div>
              {dna.file_url && (
                <button
                  ...
                  onClick={async () => {
                    try {
                      const url = await actionGetDnaSignedUrl(dna.file_url!)
                      if (url) window.open(url, '_blank', 'noopener,noreferrer')
                      else toast.error('Fichier source indisponible')
                    } catch (e: any) {
                      toast.error(e.message ?? 'Accès au fichier refusé')
                    }
                  }}
                  className="inline-flex items-center gap-1.5 mt-2 font-sans text-[10px] font-bold text-purple hover:underline"
                >
            ...
            <div className="flex flex-col items-center py-4 text-center">
              <p className="font-sans text-[11px] text-text-dim mb-3">Aucun ADN enregistré</p>
              <Link href="/campagne/etape-1">
                <button className="font-sans text-[10px] font-bold text-teal border border-border-teal rounded-neo px-3 py-1.5 hover:bg-teal/10 transition-colors">
                  + Définir l'ADN
                </button>
              </Link>
            </div>
        ...
        <div className="bg-bg-card border border-border rounded-neo-lg p-5">
          ...
          {contentTypes.length === 0 ? (
            <div className="flex flex-col items-center py-4 text-center">
              <p className="font-sans text-[11px] text-text-dim mb-3">Aucun contenu sélectionné</p>
              <Link href="/campagne/etape-2">
                <button className="font-sans text-[10px] font-bold text-accent border border-accent/40 rounded-neo px-3 py-1.5 hover:bg-accent/10 transition-colors">
            ...
            <div className="flex flex-col gap-2">
              {contentTypes.map((ct) => {
                const cfg = CONTENT_LABELS[ct.content_type] ?? { label: ct.content_type, color: 'text-text-muted border-border' }
                return (
                  <div key={ct.id} className={`flex items-center gap-2.5 px-3 py-2 rounded-neo border text-[11px] font-sans font-bold ${cfg.color}`}>
                    <span>{cfg.label}</span>
        ...
        <div className="bg-bg-card border border-border rounded-neo-lg p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {assignments.length > 0 && (
              <span className="font-sans text-[10px] font-bold text-teal">
          ...
          {assignments.length === 0 ? (
            <div className="flex flex-col items-center py-4 text-center">
              <p className="font-sans text-[11px] text-text-dim mb-3">Aucun avatar assigné</p>
              <Link href="/campagne/etape-3">
                <button className="font-sans text-[10px] font-bold text-teal border border-teal/40 rounded-neo px-3 py-1.5 hover:bg-teal/10 transition-colors">
            ...
            <div className="flex flex-col gap-2">
              {assignments.map((a) => (
                <div key={a.id} className="flex items-center gap-2.5 px-3 py-2.5 rounded-neo border border-border-teal bg-teal/5">
                  <div className="w-7 h-7 rounded-neo border border-border-teal bg-teal/10 flex items-center justify-center font-sans text-[10px] font-bold text-teal flex-shrink-0">
                    {(a.avatarName ?? '?').slice(0, 2).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-sans text-[11px] font-bold text-text-primary">{a.avatarName ?? 'Avatar'}</p>
                    {a.avatarAge && <p className="font-sans text-[9px] text-text-dim">{a.avatarAge} ans</p>}
                  </div>
                  <span className={`font-sans text-[9px] font-bold px-1.5 py-0.5 rounded border
      ...
      <div className="bg-bg-card border border-border rounded-neo-lg p-5 mb-5">
        <div className="flex items-center justify-between mb-4">
          <div>
          ...
          {campaignAssets.length > 0 && (
            <Link href="/galerie">
              <button className="font-sans text-[10px] text-accent border border-accent/40 rounded-neo px-3 py-1.5 hover:bg-accent/10 transition-colors">
        ...
        {campaignAssets.length === 0 ? (
          <div className="border border-dashed border-border rounded-neo-lg py-10 text-center">
            <div className="text-3xl mb-2">●</div>
            <p className="font-sans text-[11px] text-text-dim mb-1">Aucun contenu généré</p>
            <p className="font-sans text-[10px] text-text-dim mb-4">
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-3">
              ...
              <div key={asset.id} className="relative border border-border rounded-neo overflow-hidden bg-bg-elevated group">
                <div className="aspect-video flex items-center justify-center bg-bg-elevated">
                  {asset.type === 'image' ? (
                    ...
                    <span className="font-sans text-[10px] text-text-dim uppercase">{typeLabel(asset.type)}</span>
                  )}
                </div>
                <div className="p-2">
                  <p className="font-sans text-[9px] text-text-dim truncate">{asset.title}</p>
                  <p className="font-sans text-[8px] text-text-dim/60">{engineLabel(asset.engine)}</p>
                </div>
              </div>
            ))}
            {campaignAssets.length > 8 && (
              <Link href="/galerie" className="aspect-video flex flex-col items-center justify-center border border-dashed border-border rounded-neo hover:border-accent hover:bg-accent/5 transition-all">
                <span className="font-sans text-[10px] font-bold text-text-dim">+{campaignAssets.length - 8}</span>
                <span className="font-sans text-[9px] text-text-dim">autres</span>
              </Link>
            )}
          </div>
        )}
      ...
      {/* ── CTA selon statut ── */}
      {campaign.status === 'draft' && wizardComplete && (
        <div className="bg-bg-card border border-accent rounded-neo-lg p-5 flex items-center justify-between shadow-neo">
          <div>
      ...
      {(campaign.status === 'active' || campaign.status === 'pre_campaign') && (
        <div className="bg-bg-card border border-teal rounded-neo-lg p-5 flex items-center justify-between">
          <div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-5 animate-fade-in" onClick={() => setConfirmDelete(false)}>
          <div className="w-full max-w-[400px] bg-bg-card border border-coral rounded-neo-lg p-6 animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-display font-bold text-[16px] text-text-primary mb-2">Supprimer cette campagne ?</h3>
            <p className="font-sans text-[12px] text-text-dim mb-6 leading-relaxed">
