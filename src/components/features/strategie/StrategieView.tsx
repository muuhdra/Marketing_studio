...
const STRATEGY_TEMPLATES = [
  { id: 'hook',        label: 'Hook Émotionnel UGC',    phase: 'launch', icon: '⚡',
  ...
  { id: 'teasing',     label: 'Teasing Pré-lancement',  phase: 'pre',    icon: '🔮',
  ...
  { id: 'scale',       label: 'Scale Winning Ads',      phase: 'scale',  icon: '📈',
  ...
  { id: 'retargeting', label: 'Retargeting Post',        phase: 'post',   icon: '🎯',
  ...
  { id: 'ugc-auth',    label: 'Authenticité UGC Pure',  phase: 'launch', icon: '📱',
  ...
  { id: 'story',       label: 'Storytelling Produit',   phase: 'launch', icon: '📖',
    ...
    <div className="animate-fade-in">
      ...
      {showForm && (
        <div className="animate-fade-in mb-7">
          ...
          {/* Banner IA */}
          <div className="flex items-center gap-3 p-4 bg-purple/5 border-2 border-border-purple rounded-neo-lg mb-5">
                ...
                {STRATEGY_TEMPLATES.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-start gap-3 p-3.5 rounded-neo border-2 border-border text-left hover:border-purple hover:bg-purple/5 transition-all"
                    >
                      <span className="text-xl flex-shrink-0">{s.icon}</span>
                      <div>
                        <div className="font-mono text-[11px] font-bold text-text-primary mb-0.5">{s.label}</div>
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="bg-bg-card border-2 border-border rounded-neo-lg p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setSelected(null)} className="font-mono text-[10px] text-text-dim hover:text-accent">
                    ← Changer de template
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2.5 mb-5 p-3 bg-purple/5 border border-border-purple rounded-neo">
                  ...
                  <div>
                    <div className="font-mono text-[11px] font-bold text-purple">{strat?.label}</div>
                    <div className="font-mono text-[9px] text-text-dim">{strat?.phase} · template sélectionné</div>
                  </div>
                </div>
              ...
              {/* Loading state */}
              {generating && (
                <div className="flex flex-col items-center justify-center gap-4 bg-bg-card border-2 border-border-purple rounded-neo-lg p-8">
                  <div className="w-16 h-16 rounded-neo-lg border-2 border-border-purple bg-purple/10 flex items-center justify-center text-3xl animate-pulse">
                    🧠
                  </div>
                  <div className="text-center">
                    <p className="font-display font-bold text-[15px] text-purple mb-1">Claude réfléchit...</p>
                    <p className="font-mono text-[11px] text-text-dim">Analyse des objectifs · Construction des phases · Définition des KPIs</p>
                  </div>
                  <div className="flex gap-1">
                    {[0, 1, 2].map((i) => (
                      <div key={i} className="w-2 h-2 rounded-full bg-purple animate-bounce" style={{ animationDelay: `${i * 0.2}s` }} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {!generating && (
                <div className="bg-bg-card border-2 border-border rounded-neo-lg p-5 opacity-50">
                  ...
                  <div className="flex flex-col gap-2">
                    {['Phases & Timeline', 'KPIs & Objectifs', 'Plan de contenu hebdo', 'Conseils Claude'].map((item) => (
                      <div key={item} className="h-8 bg-bg-surface border border-border rounded-neo flex items-center px-3">
                        <span className="font-mono text-[11px] text-text-dim">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {aiResult && (
            <div className="animate-fade-in flex flex-col gap-5">
              ...
              {/* Résumé */}
              <div className="bg-bg-card border-2 border-border-purple rounded-neo-lg p-5">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-9 h-9 rounded-neo-md border-2 border-border-purple bg-purple/15 flex items-center justify-center text-lg">
                    {strat?.icon}
                  </div>
                  <div>
                    <h3 className="font-display font-bold text-[15px] text-purple">{campaignName}</h3>
                    <p className="font-mono text-[10px] text-text-dim">{strat?.label} · Généré par Claude Opus 4</p>
                  </div>
              ...
              {activeTab === 'phases' && (
                <div className="grid grid-cols-3 gap-3">
                  {aiResult.phases.map((phase, i) => (
                    <div key={i} className="bg-bg-card border-2 border-border rounded-neo-lg p-4">
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-6 h-6 rounded-neo bg-purple/20 border border-border-purple flex items-center justify-center">
                          <span className="font-mono text-[10px] font-bold text-purple">{i + 1}</span>
                        </div>
                        <div>
                          <div className="font-display font-bold text-[12.5px] text-text-primary">{phase.name}</div>
                          <div className="font-mono text-[9px] text-text-dim">{phase.duration}</div>
                        </div>
                      </div>
              ...
              {activeTab === 'kpis' && (
                <div className="grid grid-cols-2 gap-3">
                  {aiResult.kpis.map((kpi, i) => (
                    <div key={i} className="bg-bg-card border-2 border-border rounded-neo-lg p-4 flex items-center justify-between">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-2 border-border">
                        <th className="px-4 py-3 text-left font-mono text-[10px] font-bold text-text-dim">SEMAINE</th>
                        <th className="px-4 py-3 text-left font-mono text-[10px] font-bold text-text-dim">TYPE</th>
                        <th className="px-4 py-3 text-left font-mono text-[10px] font-bold text-text-dim">QTÉ</th>
                        <th className="px-4 py-3 text-left font-mono text-[10px] font-bold text-text-dim">PLATEFORME</th>
                      </tr>
                    </thead>
                    <tbody>
                      {aiResult.contentPlan.map((row, i) => (
                        <tr key={i} className="border-b border-border/50 last:border-0 hover:bg-bg-surface transition-colors">
                          <td className="px-4 py-3 font-mono text-[11px] text-text-muted">S{row.week}</td>
                          ...
                          <td className="px-4 py-3">
                            <span className="font-mono text-[10px] text-text-dim border border-border px-2 py-0.5 rounded-neo capitalize">
                              {row.platform}
                            </span>
                          </td>
                        </tr>
              ...
              {aiResult.tips.length > 0 && (
                <div className="bg-teal/5 border-2 border-border-teal rounded-neo-lg p-5">
        ...
        <>
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            <button
              ...
              className={`font-mono text-[11px] font-bold px-3 py-1.5 rounded-neo border-2 transition-all duration-100
            ...
            {Object.entries(PHASES).map(([key, p]) => (
              <button
                ...
                className={`font-mono text-[11px] font-bold px-3 py-1.5 rounded-neo border-2 transition-all duration-100
                  ${filterPhase === key
                    ? `border-${p.variant === 'accent' ? 'accent' : p.variant} text-${p.variant === 'accent' ? 'accent' : p.variant} bg-${p.variant === 'accent' ? 'accent' : p.variant}/10`
              ...
              {filtered.map((s) => {
                ...
                return (
                  <div
                    ...
                    className={`border-2 rounded-neo-lg p-4 cursor-pointer transition-all duration-150
                      ${isSel
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-9 h-9 rounded-neo-md border-2 border-border flex items-center justify-center text-lg">
                        {s.icon}
                      </div>
                    ...
                    {isSel && <div className="mt-2 font-mono text-[10px] font-bold text-accent">✓ Sélectionnée</div>}
                  </div>
                )
              })}
            </div>
              ...
              <div className="sticky top-0 flex flex-col gap-3.5 animate-slide-in">
                <div className="border-2 border-accent bg-bg-card rounded-neo-lg p-5 shadow-neo">
                  <div className="flex items-center gap-3 mb-5">
                    <div className="w-11 h-11 rounded-neo-md border-2 border-border flex items-center justify-center text-2xl flex-shrink-0">
                      {strat.icon}
                    </div>
                    <div>
